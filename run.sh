...
rfcx_dir="rfcx"
feats_dir="feats"
exp_dir="exp_ResNeStSelu_SpecAug_mixup_anti"
...
# model config
model_type="ResNeSt50" # EfficientNetB0/EfficientNetB1/EfficientNetB2/ResNeSt50
activation_type="selu" # default/mish/selu
...
if [ $INFERENCE -eq 1 ]; then
    if [ $ensemble == "True" ]; then
        if [ $antimodel == "True" ]; then
            python submit_result_copy.py --from_anti_model --feature_type $feats_type --model_type $model_type --activation $activation_type --ensemble \
            ...
            python submit_result_copy.py --feature_type $feats_type --model_type $model_type --activation $activation_type --ensemble \
                --ensembleB $exp_dir/fold1/best_model.pt \
                --ensembleC $exp_dir/fold2/best_model.pt \
                --ensembleD $exp_dir/fold3/best_model.pt \
                --ensembleE $exp_dir/fold4/best_model.pt \
        ...
        if [ $antimodel == "True" ]; then
            python submit_result_copy.py --feature_type $feats_type --model_type $model_type --activation $activation_type --from_anti_model $rfcx_dir/test/ $exp_dir/fold$test_nfold/best_model.pt
        else
            python submit_result_copy.py --feature_type $feats_type --model_type $model_type --activation $activation_type $rfcx_dir/test/ $exp_dir/fold$test_nfold/best_model.pt
        fi
    fi
fi
